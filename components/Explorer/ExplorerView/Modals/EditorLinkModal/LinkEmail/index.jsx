...
export class LinkEmail extends Component {
  ...
  componentDidMount() {
    ...
    if (selectionHasFormatting && selectionHasFormatting.attributes.link.email) {
      this.setState({
  ...
  componentWillReceiveProps(nextProps) {
    ...
    } else if (selectionHasFormatting && !selectionHasFormatting.attributes.link.email) {
      this.setState({
        href: '',
        target: false,
      });
    ...
    return (
      <div
        className={classNames.bind(styles)({
          'modal__editor-link__email': true,
        })}
      >
        <div
          className={classNames.bind(styles)({
            'modal__editor-link__email__wrapper': true,
          })}
        >
          <input
            ref={c => this.input = c}
            type="text"
            className={classNames.bind(styles)({
              'modal__editor-link__email__input': true,
            })}
          ...
          <i
            className={classNames.bind(styles)({
              'material-icons': true,
              'md-18': true,
              'modal__editor-link__email__icon': true,
            })}
          >
            contact_mail
          </i>
        ...
        <LinkSave
          onSave={e => formatContent(e, bloc.id, { href: this.input.value, email: true }, editor, content, onContentChange)}
        />
      </div>
    );
  }
...
const LinkEmailContainer = connect(
  mapStateToProps,
  mapDispatchToProps,
)(LinkEmail);

LinkEmailContainer.displayName = 'LinkEmailContainer';

export default LinkEmailContainer;
